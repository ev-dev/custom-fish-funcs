# Defined in /Users/admin/.config/fish/functions/gcl.fish @ line 2
function gcl --argument repoToClone
	if begin test -z "$repoToClone"
		...
		git clone git@gitlab.com:$repoToClone.git
		cd $newRepo
		...
		getopts $FLAG_OR_NEWNAME | while read -l key options
			switch $key
				case _
					...
					set_color normal
					git clone git@gitlab.com:$repoToClone.git $newRepoName
					cd $newRepoName
